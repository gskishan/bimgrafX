...
def _set_gl_entries_cash(
    ...
    query = apply_additional_conditions(
        "GL Entry", query, from_date, ignore_closing_entries, filters
    )

    # ✅ FIX: Removed ExistsCriterion
    if (root_lft and root_rgt) or root_type:
        account_filter = get_account_filter_query(
            root_lft, root_rgt, root_type, gl
        )
        query = query.where(account_filter)

    # Match conditions (user permissions)
    ...
    gl_entries = frappe.db.sql(query, params, as_dict=True)
...
def execute(filters=None):
    ...
    if filters.get("selected_view") == "Margin":
        compute_margin_view_data(
            data, period_list, filters.accumulated_values
        )
