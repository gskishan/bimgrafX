...
def get_columns(filters=None):
    ...
    if filters and filters.get("company"):
        currency = frappe.db.get_value("Company", filters["company"], "default_currency") or "AED"
...
# ---------------------------------------------------------------------------
# ACCRUAL BASIS  — GL Entries from all voucher types within date range
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CASH BASIS — only GL Entries where actual cash/bank movement occurred
# ---------------------------------------------------------------------------
...
def get_cash_data(filters):
    ...
    - Journal Entry      → only those linked to a payment (multi_currency or cash/bank account)
    - Sales Invoice      → only the GL entries on invoices that are fully/partially paid
                           (uses Payment Ledger Entry to find paid amounts)
    - Purchase Invoice   → same as Sales Invoice
    """
    result = {}

    # 1. Payment Entry GL entries
    _merge(result, get_payment_entry_cash_data(filters))

    # 2. Journal Entry GL entries where account is a cash/bank account
    _merge(result, get_journal_entry_cash_data(filters))

    # 3. Sales Invoice — paid portion via Payment Ledger Entry
    _merge(result, get_invoice_cash_data(filters, "Sales Invoice"))

    # 4. Purchase Invoice — paid portion via Payment Ledger Entry
    ...
    rows = frappe.db.sql(
        ...
        WHERE
            ...
            AND gl.voucher_no IN (
                ...
                WHERE ac2.account_type IN ('Cash', 'Bank')
...
def get_invoice_cash_data(filters, doctype):
    ...
    vouchers that have received/made payment within the date range,
    then pull their GL entries.
    """
    conditions = get_gl_conditions(filters)
    ple_date_conditions = get_ple_conditions(filters)
...
def _merge(target, source):
    ...
    for account, vals in source.items():
        if account not in target:
            target[account] = {"debit": 0, "credit": 0, "account_type": vals.get("account_type", "")}
